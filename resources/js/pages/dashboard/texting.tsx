...
const EmployeeList: React.FC<Props> = ({ employees: employeeData, unassignedLeads: initialUnassignedLeads = [] }) => {
    const { flash, csrf_token } = usePage().props as PageProps;
   ...
   useEffect(() => {
        ...
        console.log('CSRF Token on component mount:', token);
    }, []);
    ...
    const fetchUnassignedLeads = async () => {
        try {
            // ✅ Remove manual CSRF headers - use default fetch
    ...
    const openAssignModal = (employee: Employee) => {
        ...
        const token = document.querySelector('meta[name="csrf_token"]')?.getAttribute('content');
        console.log('CSRF Token when opening modal:', token);
    ...
    // Always get the CSRF token directly from the meta tag
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Refresh the unassigned leads list after successful assignment
            if (selectedEmployee) {
                fetchUnassignedLeads();
            }
        } else {
